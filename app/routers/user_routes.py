...
@router.post("/token", response_model=TokenResponse, tags=["Login and Registration"])
...
async def get_user(user_id: UUID, request: Request, db: AsyncSession = Depends(get_db), token: str = Depends(oauth2_scheme), current_user: dict = Depends(require_role(["ADMIN", "MANAGER"]))):
    try:
        ...
        raise HTTPException(status_code=500, detail="An unexpected error occurred while fetching the user")
...
async def list_users(
    ...
    try:
        ...
        logger.error(f"Unexpected error while listing users: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail="An unexpected error occurred while listing users")
...
async def verify_email(user_id: UUID, token: str, db: AsyncSession = Depends(get_db), email_service: EmailService = Depends(get_email_service)):
    try:
        ...
        raise HTTPException(status_code=500, detail="An unexpected error occurred during email verification")