...
async def test_create_user_access_denied(async_client, user_token):
    ...
    assert "Access denied. Admin or Manager role required." in response.json().get("detail", "")
...
async def test_create_user_duplicate_email(async_client, verified_user):
    user_data = {"email": verified_user.email, "password": "AnotherPassword123!"}
...
async def test_create_user_invalid_email(async_client):
    user_data = {"email": "notanemail", "password": "ValidPassword123!"}
    response = await async_client.post("/register/", json=user_data)
    assert response.status_code == 422

@pytest.mark.asyncio
async def test_login_success(async_client, verified_user):
    form_data = {"username": verified_user.email, "password": "VerifiedPassword123!"}
    response = await async_client.post("/token", data=urlencode(form_data), headers={"Content-Type": "application/x-www-form-urlencoded"})
    assert response.status_code == 200
    data = response.json()
...
async def test_login_user_not_found(async_client):
    form_data = {"username": "nonexistentuser@noway.com", "password": "DoesNotMatter123!"}
    response = await async_client.post("/token", data=urlencode(form_data), headers={"Content-Type": "application/x-www-form-urlencoded"})
...
@pytest.mark.asyncio
async def test_login_unverified_user(async_client, user):
    form_data = {"username": user.email, "password": "MySuperPassword$1234"}
    response = await async_client.post("/token", data=urlencode(form_data), headers={"Content-Type": "application/x-www-form-urlencoded"})
    assert response.status_code == 401
    assert "Email not verified" in response.json().get("detail", "")

@pytest.mark.asyncio
async def test_login_locked_user(async_client, user):
    user.is_locked = True
    form_data = {"username": user.email, "password": "MySuperPassword$1234"}
    response = await async_client.post("/token", data=urlencode(form_data), headers={"Content-Type": "application/x-www-form-urlencoded"})
    assert response.status_code == 400
